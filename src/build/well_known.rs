...
//! record's AT-URI. The URI is fully derivable from config — `publish.did` +
//! an rkey hashed from the site origin (see
...
/// verification is off, no `publish:` block exists, or `publish.did`/`site.url`
/// (the derivation inputs) are missing.
