...
/// secrets. Present only when the site declares `atproto:` in `config.yaml`
/// (stored as `Option<Atproto>` on [`crate::config::Config`]).
#[derive(Debug, Clone)]
pub struct Atproto {
...
/// otherwise-valid config parses, with the requirement enforced at atproto time.
...
/// Parse the `atproto:` mapping. Rejects unknown top-level keys (and unknown keys
...
pub fn parse_atproto(map: &Mapping, default_collection: &str) -> Result<Atproto> {
    reject_unknown(
        map,
...
/// message (e.g. `atproto.publication`).
