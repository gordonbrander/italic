//! The `atproto resolve-did` verb: look up the DID behind a handle, so users can
...
/// (mirroring [`crate::publish::pubstatus::run`]): it builds a current-thread
/// runtime and drives the async lookup to completion.
pub fn resolve_did(handle: &str) -> Result<()> {
