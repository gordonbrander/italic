...
**These five are the only variables.** An unknown `:token` is **silently left
literal** in the output path — Jekyll/Hugo habits like `:year`, `:month`,
`:title`, or `:categories` produce a literal `:year` directory, not an error.
And in a non-taxonomy *archive's* permalink, the date tokens expand against
the Unix epoch (`:yyyy` → `1970`) — archives have no document date.
...
**Verify:** `italic build`, then check the expected file exists — e.g.
`ls public/blog/2026/hello/index.html`. A literal `:token` directory in
`public/` means an unknown pattern variable.

## See also

- [Redirects](redirects.md) — redirect old URLs after a permalink change
- [Frontmatter reference](frontmatter.md#permalink-patterns)
- [Template reference: URL filters](url-filters.md)
