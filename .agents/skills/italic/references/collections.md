...
| Key | Default | Meaning |
|-----|---------|---------|
| `path` | — | Glob matched against paths in `content/`. `literal_separator` is on: `posts/*.md` stays in one directory; `posts/**` descends. |
...
A collection named [`all`](config.md#the-all-collection) always exists (every
doc, date descending) — you don't declare it, and it's what `sitemap:`,
`feed:`, and the `all()` function default to.
...
A `defaults:` entry must name a declared collection (a theme's collection
counts), or the build fails.
...
- [Configuration reference: collections](config.md#collections)
- [Template reference: collection()](template-functions.md#collectionname--list-a-named-collection)
