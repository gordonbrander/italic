...
1. **Resolve identity (optional but typical).** Turn a handle into a DID and find the user's PDS via the PLC directory (`com.atproto.identity.resolveHandle`, then the DID doc gives the PDS service endpoint). If you already know the PDS host, you can skip ahead. (italic ships this as `italic atproto did <handle>` — italic is DID-only elsewhere, so this is the on-ramp from a handle to `ITALIC_ATPROTO_DID`.)
