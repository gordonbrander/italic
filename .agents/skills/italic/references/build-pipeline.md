...
4. **Render markup** *(parallel)* — each body runs through a markup-phase
...
through Tera *before* the index is complete, so markup-phase templates can't
...
[template reference](template-context.md#the-two-phases) marks each
...
markup-phase template inside a document body already sees them (e.g.
