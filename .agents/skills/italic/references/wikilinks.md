...
This page covers `[[wikilink]]` syntax, how targets resolve, block references,
media embeds, the backlink graph, and where italic's behavior differs from
Obsidian's.
...
[template reference](doc-list-filters.md#backlinks--pages-that-link-to-this-one).
...
All verified against the current binary:

- **`![[Note]]` does not transclude.** Embeds work for co-located **media
  only** (`![[pic.png]]` → `<img class="embed">`; see
  [Authoring](authoring.md#co-located-media-images-and-attachments)). An embed of another *note* is left as
  literal `![[Note]]` text in the output.
- **`[[note.md]]` (extension form) does not resolve** — it renders as
  `span.nolink`. Link by stem: `[[note]]`.
- **`aliases:` frontmatter is inert.** `[[Some Alias]]` does not resolve to
  the note that declares it; only filename stems (and `dir/Name` prefixes)
  resolve.
- **`%%comments%%` are not stripped** — they render literally into the
  published page. Remove them before publishing.
- **Unresolved links don't create stub pages** — they render as `span.nolink`
  and nothing else.
- Plain Markdown `[label](other.md)` links don't register backlinks (above).

## See also

- [Related pages](related.md)
- [Template reference: backlinks](doc-list-filters.md#backlinks--pages-that-link-to-this-one)
