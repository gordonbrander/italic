...
With no `template:` key, the rendered body **is** the final output — same rule
as documents. Add `template: some-layout.html` to wrap the archive body in a
layout (the layout sees the same `pagination`/`term` context).
...
[query](config.md#collections), and is applied per term, just before
pagination:
...
`/feed/all.xml`, configurable with the [`sitemap`](config.md#sitemap)
and [`feed`](config.md#feed) config keys (each can name other
...
**Verify:** `italic build`, then check the emitted tree — e.g.
`ls public/blog/ public/tags/ public/feed/` should show `index.html`,
one directory per term, and `<collection>.xml` respectively.
...
- [Frontmatter reference: archive keys](frontmatter.md#archive-keys)
